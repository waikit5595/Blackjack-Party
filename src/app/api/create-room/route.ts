...
export async function POST(request: NextRequest) {
  try {
    ...
    await adminDb.ref(`rooms/${roomCode}`).set({ roomCode, hostUid: uid, status: "waiting", createdAt: now, dealerSeat: 12, revealAll: false, currentRound: 0, currentTurnSeat: null, turnOrder: [], deck: [], seats: { ...buildEmptySeats(), "12": uid }, players: { [uid]: player }, hands: {} });
